package com.styleway.app.services;
...
import com.styleway.app.activities.ChatActivity;
...
import com.styleway.app.utils.FirebaseUploader;
...
public class FirebaseChatService extends Service {
    ...
    public void onCreate() {
        ...
        Notification notification = new NotificationCompat.Builder(this, CHANNEL_ID_MAIN)
                .setSmallIcon(R.drawable.style_way)
    ...
    private BroadcastReceiver myUsersReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (myUsers != null) {
                RealmResults<User> toDelete = rChatDb.where(User.class).notEqualTo("id", userMe.getId()).findAll();
                if (toDelete != null) {
    ...
    private BroadcastReceiver myContactsReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (myContacts != null) {
                ...
                helper.setMyUsersNameCache(myContacts);
            }
        }
    };
            ...
            inboxRef.child(myId).addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    try {
                        ...
                        if (message != null && !message.getSenderId().equals(userMe.getId()) && !registeredChatUpdatedUserIds.contains(message.getSenderId())) {
    ...
    private void notifyNewGroup(Group group) {
        ...
        notificationBuilder.setSmallIcon(R.drawable.style_way)
    ...
    private boolean updateUserInDb(final User user) {
        User inDb = rChatDb.where(User.class).equalTo("id", user.getId()).findFirst();
        if (inDb != null && (!inDb.getImage().equals(user.getImage()) || !inDb.getStatus().equals(user.getStatus()) || !inDb.getName().equals(user.getName()))) {
            String idTrim = Helper.getEndTrim(user.getId());
            user.setNameInPhone(myContacts.containsKey(idTrim) ? myContacts.get(idTrim).getName() : user.getName());
            rChatDb.beginTransaction();
            rChatDb.copyToRealmOrUpdate(user);
            rChatDb.commitTransaction();
